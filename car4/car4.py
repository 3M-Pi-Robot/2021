...
trimfactor_ena = 1.01 # adjust the speed 
trimfactor_enb = 1.01
...
while True:
    ...
    for event in pygame.event.get():
        if event.type == KEYDOWN and event.key == K_ESCAPE:
            ...
            pygame.display.quit()
            sys.exit
        elif event.type == KEYDOWN and event.key == K_w: # forward
            print("w - forward")
            ena +=15
            enb +=15
        ...
        if(ena>=0):            
            ...
            ena_minus = abs(ena)
            ena_minus *= trimfactor_ena
            ena_minus = min(ena_minus, 100)
            pin.Level("ENA",ena_minus)
            pin.Out("IN1",1)
            pin.Out("IN2",0)
        if(enb>=0):
            enb_trim = enb * trimfactor_enb
            enb = min(enb, 100)
            pin.Level("ENB",enb)
            pin.Out("IN3",1)
            pin.Out("IN4",0)
